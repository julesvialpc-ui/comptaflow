...
function MessageBubble({ msg, isStreaming }: { msg: ChatMessage & { streaming?: boolean }; isStreaming?: boolean }) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'} group`}>
      {/* Avatar */}
      {isUser ? (
        <div className="flex-shrink-0 h-8 w-8 rounded-full bg-indigo-600 flex items-center justify-center text-xs text-white font-bold">
          Moi
        </div>
      ) : (
...
function ToolIndicator({ tools }: { tools: string[] }) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-xl px-3 py-2 w-fit">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 w-full max-w-2xl">
              {SUGGESTIONS.map((s) => (
                <button
                  ...
                  className="flex items-center gap-2.5 p-3 bg-white border border-zinc-200 rounded-xl text-left text-sm hover:border-indigo-300 hover:bg-indigo-50 hover:shadow-sm transition-all group"
                >
                  <span className="text-xl">{s.icon}</span>
                  <span className="text-zinc-700 group-hover:text-indigo-700 text-xs font-medium">{s.label}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="border-t border-zinc-200 bg-white px-4 sm:px-6 py-3">
          ...
          <div className="flex gap-2 mb-2 overflow-x-auto pb-1 scrollbar-hide">
            {SUGGESTIONS.slice(0, 4).map((s) => (
              <button
                key={s.label}
                onClick={() => sendMessage(s.prompt)}
                className="flex-shrink-0 flex items-center gap-1.5 text-xs bg-zinc-50 border border-zinc-200 rounded-full px-3 py-1.5 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 transition-colors text-zinc-600"
              >
                <span>{s.icon}</span>
                <span>{s.label}</span>
              </button>
        ...
        <div className="flex items-end gap-3">
          ...
          <button
            ...
            className="flex-shrink-0 h-11 w-11 rounded-xl bg-indigo-600 text-white flex items-center justify-center shadow hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            {loading ? (
